...
/* ===== SIGNALS TOPBAR — HIDE ON SCROLL ===== */
(() => {
  const scrollContainer = document.querySelector(".content");
  const app = document.querySelector(".pulse-app");

  if (!scrollContainer || !app) return;
  ...
  function updateTopbar() {
    ...
    if (currentScrollTop <= 4) {
      app.classList.remove("is-topbar-hidden");
      accumulatedDelta = 0;
      lastDirection = 0;
    ...
    if (direction > 0 && currentScrollTop > HIDE_AFTER && accumulatedDelta >= DIRECTION_THRESHOLD) {
      app.classList.add("is-topbar-hidden");
      accumulatedDelta = 0;
    } else if (direction < 0 && Math.abs(accumulatedDelta) >= DIRECTION_THRESHOLD) {
      app.classList.remove("is-topbar-hidden");
      accumulatedDelta = 0;
    }
